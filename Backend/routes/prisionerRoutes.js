...
//  console.log(prev_month_date, this_month)
    ...
    // console.log('Prisioner Route Line:486', prev_month_date, this_month)
    ...
    const sql = `SELECT 
        ...
        SUM(CASE WHEN (pi.release_id IS NULL AND pi.gender='M') OR (pi.release_id IS NOT NULL AND pi.released_date BETWEEN '${formatted_prev_month}' AND '${this_month}' AND pi.gender='M') THEN 1 ELSE 0 END) AS TotalPrevMaleMonth,
...
router.get('/get_prisoners_report', verifyToken, async (req, res) => {
    const userToken = req.user; // Extract details from the token
    const { startDate, endDate, type } = req.query;  // Add 'type' to differentiate between Nepali and foreign prisoners
    
    const isNepali = type === 'nepali';  // Check if the query is for Nepali prisoners or foreign prisoners
    const countryCondition = isNepali ? "npc.name_np='नेपाल' OR pi.country=154" : "npc.name_np!='नेपाल' OR pi.country!=154";
...
LEFT JOIN np_country npc ON pi.country = npc.id
WHERE
    pi.office_id = ? AND pi.karagar_date <= ? AND (${countryCondition})
